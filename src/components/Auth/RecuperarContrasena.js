...
function Recuperar() {
    ...
    const recuperar = async()=>{
        if(email){
        let usuario = RecuperarContra(email);
    ...
    const verificacionPin = ()=>{
        if(pin === usuario.pin){
            swal({ title: 'Cambia tu contraseña', icon: 'success', button: 'Aceptar', closeOnClickOutside: false, closeOnEsc: false });
            setAccion(3);
        }
        else{
                            ...
                            <div className="container">
                                ...
                                <div className="row">
                                    <div className="col-lg-10 col-xl-7 mx-auto">
                                        ...
                                        <form>
                                            <div className="form-group mb-3">
                                                <input id="inputEmail" type="text" placeholder="Pin" className="form-control rounded-pill border-0 shadow-sm px-4" onChange={(event) => setPin(event.target.value)}/>
                                            </div>
                                            <span className="btn btn-primary btn-block text-uppercase mb-2 rounded-pill shadow-sm" onClick={()=>{
                                                verificacionPin();
                                            }}>Verificar Pin</span>
